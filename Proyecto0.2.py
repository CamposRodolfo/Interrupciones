...
def CalcularDiagramaControlProcesos(T0, Tf):
    ...
    tiempo_actual = T0
    while tiempo_actual <= Tf:
        fila = [f"T={tiempo_actual}"]
        for dispositivo in dispositivos:
            interrupciones = [row for row in TablaDatos if row[0] == tiempo_actual and row[1] == dispositivo]
            if interrupciones:
                for interrupcion in interrupciones:
                    fila.append(f"T={tiempo_actual} ({interrupcion[2]} segundos)")
            else:
                fila.append("")
